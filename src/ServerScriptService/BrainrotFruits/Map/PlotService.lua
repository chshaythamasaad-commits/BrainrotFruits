...
local function resetPlotSlots(plot)
	...
	for _, slot in ipairs(slots:GetChildren()) do
		if slot:IsA("BasePart") then
			...
			slot:SetAttribute("IncomePerSecond", 0)
			slot:SetAttribute("FruitRarity", "")
			slot:SetAttribute("CharacterId", "")
			slot:SetAttribute("VariantId", "")
...
function PlotService.placeRewardOnSlot(player, rewardModel, reward)
	...
	slot:SetAttribute("VariantId", reward.variantName or reward.variant or "")
	slot:SetAttribute("CharacterId", reward.characterId or rewardModel:GetAttribute("CharacterId") or "Strawberita")
	slot:SetAttribute("FruitDisplayName", reward.displayName or rewardModel.Name)
	slot:SetAttribute("FruitRarity", reward.rarity or rewardModel:GetAttribute("Rarity") or "")
	slot:SetAttribute("IncomePerSecond", reward.incomePerSecond or reward.income or 0)
	...
	CharacterAnimationService.stopIdle(rewardModel)
	...
	CharacterAnimationService.startIdle(rewardModel, reward)
	DisplayPlatformService.applyDisplayPresentation(rewardModel, slot, reward)
